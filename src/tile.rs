...
impl Tile{
    ...
    pub fn draw(&self,canvas : &mut sdl2::render::Canvas<sdl2::video::Window>, texture_map : &std::collections::HashMap<String,sdl2::render::Texture>, camera : &Camera){
        ...
        if self.x as f64-camera.x+(scaled_w as f64)< 0.0 || self.y as f64-camera.y+(scaled_h as f64) < 0.0 || self.x as f64-camera.x > SCREEN_WIDTH as f64 || self.y as f64-camera.y > SCREEN_HEIGHT as f64{
            return;
        }
        match self.texture_data {
            Some(ref texture_data) => {
                let res = texture_data.draw(canvas,texture_map,self.x as f64-camera.x,self.y as f64-camera.y,scaled_w ,scaled_h);
                match res {
                    Err(..) => {
                        canvas.set_draw_color(sdl2::pixels::Color::RGB(0,0,0));                        
                        canvas.fill_rect(sdl2::rect::Rect::new((self.x as f64-camera.x)as i32,(self.y as f64-camera.y) as i32, scaled_w,scaled_h)).unwrap();
                    },
                    Ok(..) => ()
                }
            },
