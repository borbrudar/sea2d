    ...
    fn load_layer(&mut self, path : String, texture_creator : & 'a TextureCreator<sdl2::video::WindowContext>, texture_map : &mut HashMap<String,Texture<'a>>, exits : &mut Vec<String>){
        ...
        for y in 0..height{
            for x in 0..width{
                ...
                if pixel[3] >= 128 {
                    default_bounding_box = Some(AABB::new((x * self.tile_size) as f64, (y * self.tile_size) as f64, self.tile_size as u32, self.tile_size as u32));
                }
                let pixel = (pixel[0],pixel[1],pixel[2]);
                //println!("Pixel: {:?}",pixel);
                let pos :Point<i32>= Point::new((x * self.tile_size) as i32, (y * self.tile_size) as i32);

                match pixel{
                    TileType::STONE_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Stone, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/tile.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::WATER_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Water, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/water.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::GRASS_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Grass, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/grass.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::SAND_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Sand, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/sand.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::ROCK_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Rock, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/rock.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::TREE_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Tree, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/tree.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    TileType::WALL_COLOR => {
                        layer.insert(pos, Tile::new( pos.x, pos.y, self.tile_size as u32, TileType::Wall, default_bounding_box));
                        layer.get_mut(&pos).unwrap().texture_data = Some(TextureData::new("resources/textures/wall.png".to_string()));
                        layer.get_mut(&pos).unwrap().texture_data.as_mut().unwrap().load_texture(&texture_creator, texture_map);
                    },
                    ...
                    TileType::EXIT_COLOR => {
                        ...
                        let exit_bb = Some(AABB::new((x * self.tile_size + self.tile_size/4) as f64, (y * self.tile_size+self.tile_size/4)as f64, self.tile_size as u32/2, self.tile_size as u32/2));
                        layer.insert(pos, Tile::new(pos.x,pos.y,self.tile_size as u32,TileType::Exit(ExitTile{next_level : last}),exit_bb));
                    },
                    _ => ()
                }
            }
